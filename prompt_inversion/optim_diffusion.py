...
from ldm.modules.encoders.modules import NewFrozenCLIPEmbedder,FrozenOpenCLIPImageEmbedder
...
def initialize_prompt(tokenizer, token_embedding, args, device, initial_prompt_text=None,compress_features=None):
    ...
    #     prompt_len = prompt_ids.size(1)
    # else:
    #     # If no initial prompt is provided, default to a random length
    #     prompt_len = args.prompt_len
    # # Randomly generate prompt embeddings with the same length as the input prompt's token count
    # prompt_ids = torch.randint(len(tokenizer.encoder), (args.prompt_bs, prompt_len)).to(device)
...
def optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt,compress_features):
    ...
    # compress_image = diffusion.decode_first_stage(compressed_img)
    # compress_image = torch.clamp((compress_image + 1.0) / 2.0, min=0.0, max=1.0)
    

    target_image = transforms.ToTensor()(target_images).unsqueeze(0).to(device)
    bsz = target_image.shape[0]

    # res = target_image - compress_image
    # res = res.clamp_(0,1)
    ...
    ddim_timesteps = make_ddim_timesteps(ddim_discr_method="uniform", num_ddim_timesteps=20,
    ...
    for step in range(opt_iters):
        ...
        # if step % 10 == 0:
        #     projected_embeds=projected_embed
        # else:
        #     projected_embeds=prompt_embeds
        # projected_embeds = prompt_embeds
        # if step % 500== 0:
        ...
        logits_per_image, logits_per_text = model.forward_text_embedding(padded_embeds2, dummy_ids, all_target_features)
...
# torch.cat((text_embeddings, ori_embeddings),dim=1)
        ...
        #loss_x0 = torch.mean(torch.nn.functional.mse_loss(x, pred_x0, reduction='none').mean([1, 2, 3]))

        loss = loss + loss_diffusion * args.loss_diffusion
        # if step % 10 == 0:
        #     writer.add_scalar('Loss/train', loss.item(), step)

        #     loss_values.append((step, loss.item()))
        ...
        if step % args.eval_step == 0:
            ...
            # _, nn_indices = nn_project(prompt_embeds, token_embedding)
    ...
    # writer.close()
    # with open('loss_values.csv', mode='w', newline='') as file:
    #     writer = csv.writer(file)
    #     writer.writerow(['Step', 'Loss']) 
    #     writer.writerows(loss_values)
    # plt.figure(figsize=(10, 6))
    # plt.plot(loss_values2, label='Loss Curve')
    # plt.xlabel('Optimization Steps')
    # plt.ylabel('Loss Value')
    # plt.title('Loss Curve During Prompt Optimization')
    # plt.legend()
    # plt.grid(True)
    # plt.show()
...
def optimize_prompt(model, diffusion, preprocess, args, device, compressed_img, target_images=None, compress_img=None, prompt=None, target_prompts=None):
    ...
    #target_image = transforms.ToTensor()(target_images).unsqueeze(0).to(device)
    # ori_batch = preprocess(target_images).unsqueeze(0).to(device)
    # ori_feat = image_encoder.forward(ori_batch)
    # print(ori_feat.shape)
    # optimize prompt
...
# def cut_prompt(model, diffusion, preprocess, args, device, compressed_img, target_images=None, compress_img=None, prompt=None, target_prompts=None):
#     #image_encoder=FrozenOpenCLIPImageEmbedder()
#     token_embedding = model.token_embedding
#     tokenizer = open_clip.tokenizer._tokenizer
#     tokenizer_funct = open_clip.get_tokenizer(args.clip_model)
...
#     #all_target_features = get_target_feature(model, preprocess, tokenizer_funct, device, target_images=target_images, target_prompts=target_prompts)
#     compress_features = get_target_feature(model, preprocess, tokenizer_funct, device, target_images=compress_img, target_prompts=target_prompts)
#     # prompt_embeds, ids = cut_token(model,tokenizer, token_embedding, args, device, prompt, compress_features)
#     # decoded_text = decode_ids(ids, tokenizer)[0]
#     return decoded_text,prompt_embeds
