...
from cldm.ddim_hzc import DDIMSampler
...
def optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt,compress_features,opt):
    ...
    best_text = ""
    best_embeds = None
    ddim_timesteps = make_ddim_timesteps(ddim_discr_method="uniform", num_ddim_timesteps=opt.ddim_steps,
                                                  num_ddpm_timesteps=diffusion.add_steps, verbose=False)
    
    make_ddim_timesteps(ddim_discr_method="uniform", num_ddim_timesteps=50,
        ...
        if step % args.eval_step == 0:
            ...
            with torch.no_grad():
                with diffusion.ema_scope():
                    ...
                    sampler.make_schedule(ddim_num_steps=10, ddim_eta=1.0, verbose=False)
                    t_enc = 50
                    control_res = compressed_img
                    x_samples = sampler.decode(latent_x, cond_i,control_res, unconditional_guidance_scale=opt.scale,
...
def optimize_prompt(model, diffusion, preprocess, args, device, compressed_img, opt,target_images=None, compress_img=None, prompt=None, target_prompts=None):
    ...
    # optimize prompt
    learned_prompt, emb, best_rec = optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt,compress_features,opt)
