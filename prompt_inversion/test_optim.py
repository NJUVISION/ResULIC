...
def optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt, type, Q):
    # seed_everything(42)
    ...
    for step in range(opt_iters):
        ...
        loss_diffusion, pred_x0, pred_x0 = diffusion.p_losses(x, cond, timesteps)
        ...
        if step % args.eval_step == 0:
            ...
            with torch.no_grad():
                ...
                t_enc = 50
                x_samples = sampler.decode(latent_x, cond_i, t_enc, unconditional_guidance_scale=6,
...
def optimize_prompt(model, diffusion, preprocess, args, device, compressed_img, type, Q, target_images=None,  prompt=None, target_prompts=None):
    ...
    learned_prompt, emb, best_rec = optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt, type, Q)
