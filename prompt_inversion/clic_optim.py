...
def initialize_prompt(tokenizer, token_embedding, args, device, initial_prompt_text=None):
    ...
    #prompt_embeds = torch.load('/workspace/test/ControlNet/text_embeddings23.pt').to(device)
...
def optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt, concat_emb, type, Q):
    # seed_everything(42)
    ...
    target_img = transforms.functional.resize(target_images, (512, 512), Image.BICUBIC)
    target_img = transforms.ToTensor()(target_img).unsqueeze(0).to(device)
    transform = transforms.Compose([
            transforms.CenterCrop(768),
            transforms.ToTensor()
        ])
    target_image = transform(target_images).unsqueeze(0).to(device)
    print(target_image.shape)
    bsz = target_image.shape[0]


    prompt_embeds, dummy_embeds, dummy_ids, prompt_len = initialize_prompt(tokenizer, token_embedding, args, device, prompt)
    ...
    for step in range(opt_iters):
        ...
        x = diffusion.get_first_stage_encoding(diffusion.encode_first_stage(target_img)).detach()
        ...
        sample_path = f'./optimize/{type}_{Q}'
        ...
        if step % args.eval_step == 0:
            ...
            with torch.no_grad():
                ...
                latent_x = torch.randn_like(compressed_img).to(device)
...
def optimize_prompt(model, diffusion, preprocess, args, device, compressed_img, concat_emb, type, Q, target_images=None,  prompt=None, target_prompts=None):
    ...
    learned_prompt, emb, best_rec = optimize_prompt_loop(model, diffusion, preprocess, tokenizer, token_embedding, compressed_img, target_images, all_target_features, args, device, prompt, concat_emb, type, Q)
