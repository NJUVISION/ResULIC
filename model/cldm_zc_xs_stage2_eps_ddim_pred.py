...
class ControlLDM(LatentDiffusion):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        rec_cat = torch.cat((pred_x0, c_latent), dim=0)
        rec= self.decode_first_stage(rec_cat)
        # with torch.no_grad():
        x_ori = torch.cat((c_latent,c_latent), dim=0)
        rec_loss, p_loss, p_loss_d, perceptual_loss, percptual_loss_d = self.perceptual_loss(rec,x_ori)
        ...
        loss_dict.update({f'{prefix}/perceptual_loss_d': p_loss_d.mean()})
        loss += self.l_guide_weight * loss_guide
        loss += perceptual_loss
        loss += percptual_loss_d
